...
function valTimesIndex(arr){
    return arr.map(function(val,i){return val*i})
...
function extractFullName(arr){
    return arr.map(function(val){return val.first + " " + val.last})
...
function removeVowels(str) {
    str = str.toLowerCase()
    arr= Array.from(str)
    const noVowel = arr.filter(function(val,i,arr){
...
function doubleOddNumbers(arr) {
    const oddNums = arr.filter(function(val){
        return !(val%2 ===0)
    }) 
