...
function vowelCount(str) {
    ...
    let count = {}
    return strArray.reduce(function(accum, nextValue){
        if ("aeiou".includes(nextValue)===true){
        count[nextValue]? count[nextValue]++ :  count[nextValue] = 1}
...
function partition(arr, callback) {
    let partArray = [[],[]]
    return arr.reduce(function(acuum, nextValue){
        callback(nextValue)? acuum[0].push(nextValue) : acuum[1].push(nextValue)
        return acuum
    }, partArray)
}
